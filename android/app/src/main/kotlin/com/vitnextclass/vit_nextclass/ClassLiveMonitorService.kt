 ...
 * Uses [ClassLiveScheduler] alarms at class boundaries instead of 60s polling.
 * Foreground runs only during an active class or a short pre-class window.
 */
class ClassLiveMonitorService : Service() {

    companion object {
        const val ACTION_SYNC = "com.vitnextclass.CLASS_LIVE_SYNC"
        const val ACTION_STOP = "com.vitnextclass.CLASS_LIVE_STOP"
        const val ACTION_BOUNDARY = "com.vitnextclass.CLASS_LIVE_BOUNDARY"
        ...
        /** In-memory display labels (not persisted to SharedPreferences). */
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_BOUNDARY -> {
                // Re-evaluate at alarm boundary; display cache may be empty after reboot
            }
            else -> {
                // Boot or sticky restart without extras
            }
        }

        // Satisfy foreground-service contract when started via startForegroundService
        if (!isForeground) {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description =
                    "Shows your current class on the status bar on supported devices"
                setShowBadge(false)
            }
    ...
    private fun shouldRunForeground(
        ...
        // Auto-silent only: no foreground between classes (alarms handle transitions)
